...
p_hat(1,:,:) = movingPressureAmps(kappa_i,kappa_j,l,b,L,B,D,...
    ...
    (g+gamma*kappa.^2/rho);
eta_hat = -p_hat; % initial free-surface elevation coefficients
phi_hat = zeros(N,J,I); % initial velocity potential coefficients
...
for n=1:N-1
    ...
    phi_hat(n+1,:,:) = ((1-(dt*omega/2).^2).*squeeze(phi_hat(n,:,:))-...
        ...
        (squeeze(2*eta_hat(n,:,:)+...
        p_hat(n+1,:,:)+p_hat(n,:,:))))./(1+(dt*omega/2).^2); % Eq. (33);
