...
if isRefl
    kappa_i = ones(J,1)*(0:I-1)*pi/l; % Eq. (xx)
    kappa_j = (0:J-1)'*ones(1,I)*pi/b; % Eq. (xx)
else
    kappa_i = ones(J,1)*(-I/2:I/2-1)*2*pi/l; % Eq. (xx)
    kappa_j = (-J/2:J/2-1)'*ones(1,I)*2*pi/b; % Eq. (xx)
end
kappa = sqrt(kappa_i.^2+kappa_j.^2); % Eq. (xx)
kappatanhkappad = kappa.*tanh(kappa*d);
omega = sqrt((g+gamma*kappa.*kappa/rho)...
    .*kappatanhkappad); % Eq. (xx)
% moving pressure term
N = length(X);
p_hat_p = movingPressureAmps(kappa_i,kappa_j,l,b,L,B,D,...
    theta(1),X(1),Y(1),X_m0,Y_m0,theta_m0,isRefl)*g./...
    (g+gamma*kappa.^2/rho);
eta_hat_p = -p_hat_p; % initial free-surface elevation coefficients
phi_hat_p = zeros(J,I); % initial velocity potential coefficients
...
for n=1:N-1
    ...
    eta_hat = (eta_hat_p-(dt*omega/2).^2.*(eta_hat_p+...
        ...
        (1+(dt*omega/2).^2); % Eq. (32)
    phi_hat = ((1-(dt*omega/2).^2).*phi_hat_p-...
        dt/2.*(g+gamma*kappa.*kappa/rho).*...
        (2*eta_hat_p+p_hat+p_hat_p))./(1+(dt*omega/2).^2); % Eq. (33);
